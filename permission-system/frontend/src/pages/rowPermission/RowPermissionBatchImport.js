...
import { batchCreateRowPermissions } from '../../api/rowPermission';

const { TextArea } = Input;
const { Paragraph, Text } = Typography;

const RowPermissionBatchImport = ({ visible, onCancel, onSuccess }) => {
  ...
  const parseInputData = (text) => {
    ...
    lines.forEach((line, index) => {
      // 使用制表符或逗号分隔，但需要注意行过滤条件可能包含逗号，所以最多分割5部分
      const parts = line.split(/[,\t]+/, 5).map(part => part.trim());
      
      // 检查每行至少有3个字段（数据库名、表名和行过滤条件是必须的）
      if (parts.length < 3) {
        errors.push(`第 ${index + 1} 行: 格式不正确，至少需要数据库名、表名和行过滤条件`);
        return;
      }

      // 解析字段
      const [db_name, table_name, row_filter, user_names = '', role_names = ''] = parts;
      ...
      if (!row_filter) {
        errors.push(`第 ${index + 1} 行: 行过滤条件不能为空`);
        return;
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      // 调用批量创建API
      await batchCreateRowPermissions(parseResult.data);
      message.success(`成功导入 ${parseResult.data.length} 条行权限记录`);
      form.resetFields();
      onSuccess();
    } catch (error) {
      ...
      <Alert
        ...
        description={
          <div>
            ...
            <Paragraph>
              <Text code>数据库名,表名,行过滤条件,用户名1+用户名2+...,角色名1+角色名2+...</Text>
            </Paragraph>
            <Paragraph>
              说明：
              <ul>
                <li>字段之间使用逗号或制表符分隔</li>
                <li>数据库名、表名和行过滤条件为必填项</li>
                <li>用户名和角色名至少填一个</li>
                <li>多个用户名或角色名之间使用加号(+)分隔</li>
                <li>示例：<Text code>test_db,users,dept_id=10,admin+guest,</Text> 或 <Text code>test_db,orders,status='active',user1,manager+viewer</Text></li>
                <li>多用户多角色将一一对应创建权限记录，例如 <Text code>test_db,users,dept_id=10,admin+guest,manager+viewer</Text> 会创建2条记录：admin-manager和guest-viewer</li>
...
export default RowPermissionBatchImport;
